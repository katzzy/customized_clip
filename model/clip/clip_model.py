...
def load_clip_model(model_config_path: str):
    ...
    pretrained_weights_path = model_config["pretrained_weights_path"]
    model_config.pop("pretrained_weights_path")
    ...
    for key in list(pretrained_state_dict.keys()):
        if key in text_keys:
            new_key = f"textual.{key}"
            pretrained_state_dict[new_key] = pretrained_state_dict.pop(key)

    if is_state_dict_compatible(model_state_dict, pretrained_state_dict):
        print("State_dict is compatible!")
...
def is_state_dict_compatible(model_state_dict: dict, pretrained_state_dict: dict):
    ...
    if extra_keys:
        ...
        print(f"Total extra keys: {len(extra_keys)}")
    if missing_keys:
        print(f"Missing keys in model_state_dict: {missing_keys}")
        print(f"Total missing keys: {len(missing_keys)}")

    return not extra_keys and not missing_keys


def main():
    model = load_clip_model("model/model_configs/ViT-L-14.yaml")
    print(model)
